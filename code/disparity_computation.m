function disparity_map = disparity_computation(I1,I2,disparity_range,varargin)
...
%   Pattern Analysis and Machine Intelligence, IEEE Transactions on, 30(2):328ï¿??41, 2008."
%% Input parser
P = inputParser;
% Block size for simple block matching
P.addOptional('block_size',5); 
% Plot oder nicht
...
params.disparity_range = disparity_range; 
...
disparity_map = Cal_disparity( I1, I2, params );

%% plot the result
if do_plot
    figure;
    imshow(disparity_map,params.disparity_range);
    title('disparity_computation');
    colormap (gca,jet)
    colorbar
end
end
...
function disparity_map = Cal_disparity( im_left, im_right, params )
% Compute disparity map with semi global matching algorithm.
% "Heiko Hirschmuller. Stereo processing by semiglobal matching and mutual information. 
%   Pattern Analysis and Machine Intelligence, IEEE Transactions on, 30(2):328?41, 2008."
%
...
max_b = size_x + size_y;
%Initialize output
Lr = zeros(size(C), 'single');
tic;
%Iterate over direction
for dir = 1:directions_num
    disp(dir);
    Li = zeros(size(C), 'single'); %temp buffer  
...
const_grades = zeros(nLabels, 'single');
...
% Inputs:   im_left - left image, gray scale, will be converted to single.
%               size: WxHx1
%           im_right - right image, gray scale, will be converted to single
%               size: WxHx1
...
assert(block_size > 1); 
assert(disparity_range(2) > disparity_range(1)); 
if (~isa(im_reference,'single') || ~isa(im_support,'single'))
    im_reference = im2single(im_reference);
    im_support  = im2single(im_support);
end
% output image:
C = zeros(size(im_support,1), size(im_reference,2), disparity_range(2) - disparity_range(1) + 1, 'single');
% allocate im_diff. im_diff is in left coords:
im_diff = zeros(size(im_reference, 1), size(im_reference,2), 'single');
